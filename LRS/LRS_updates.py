...
class DynSegFeature:
    ...
    def update_speed_limit_neighbourhood_segmentation(
            self,
            segmented_feature_name: str = SDE_SPEED_LIMIT_SEGMENTED_FEATURE_NAME,
            neighbourhood_event_name: str = "SDEADM.E_SpeedLimit_Neighborhood",
    ):
        ...
        if neighbourhood_event_name != "SDEADM.E_SpeedLimit_Neighborhood":
            event_tables = [
                rf'{self.sde_workspace}\SDEADM.TRNLRS\SDEADM.E_StreetClass',
                rf'{self.sde_workspace}\SDEADM.TRNLRS\SDEADM.E_SpeedLimit',
    ...
    def update_speed_limit_neighbourhood(
        ...
        ROUTE_ID, STR_NAME, FULL_NAME, STR_CLASS, SPEED, REVIEW_STAT, and
        DATE_EFFECTIVE. The source is the speed-limit-specific segmented feature
        generated by :meth:`update_speed_limit_neighbourhood_segmentation`.
        """

        if out_feature is None:
            out_feature = os.path.join(self.sde_workspace, SPEED_LIMIT_NEIGHBOURHOOD_FEATURE_NAME)
