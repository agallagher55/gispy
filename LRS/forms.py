...
if __name__ == "__main__":
    ...
    for workspace in [
        ...
        for sheet_name in event_names:
            ...
            output_event_feature = os.path.join(workspace, "SDEADM.TRNLRS", f"SDEADM.{event_name}")
            if ".gdb" in workspace:
                output_event_feature = os.path.join(workspace, "TRNLRS", event_name)
            ...
            for info in form_field_info:
                ...
                if field_name not in LrsEventForm.standard_fields:
                    ...
                    # TODO: get field defaults and add
            ...
            # Register as branch versioned - Watch that feature doesn't get tagged as traditionally versioned by default
